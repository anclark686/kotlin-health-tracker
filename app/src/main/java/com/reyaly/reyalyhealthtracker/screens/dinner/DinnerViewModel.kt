...
private const val TAG = "dinnerForm"

class DinnerViewModel: ViewModel() {
    val auth: FirebaseAuth = FirebaseAuth.getInstance()

    private val _uiState = MutableStateFlow(DinnerUiState())
    ...
    suspend fun getUsersMeals(date: LocalDate) {
        ...
        try {
            val foods = findMealsInDates(firebaseUser.uid, "dinner", date).toMutableList().also {
    ...
    suspend fun onDeleteFoodInDates(food: FoodItem, date: LocalDate) {
        ...
        Log.d("edit", food.toString())
        try {
            deleteFoodFromDates(firebaseUser.uid, food.name, "dinner", date)
        } catch (e: Exception) {
            Log.d(TAG, "an error occurred: $e")
        }
    }
