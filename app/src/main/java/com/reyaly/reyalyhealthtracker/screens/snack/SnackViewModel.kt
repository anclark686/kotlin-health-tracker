...
class SnackViewModel: ViewModel() {
    ...
    suspend fun getUsersMeals(date: LocalDate) {
        ...
        try {
            val foods = findMealsInDates(firebaseUser.uid, "snacks", date).toMutableList().also {
    ...
    suspend fun onDeleteFoodInDates(food: FoodItem, date: LocalDate) {
        ...
        Log.d("edit", food.toString())
        try {
            deleteFoodFromDates(firebaseUser.uid, food.name, "snacks", date)
        } catch (e: Exception) {
            Log.d(TAG, "an error occurred: $e")
        }
    }
